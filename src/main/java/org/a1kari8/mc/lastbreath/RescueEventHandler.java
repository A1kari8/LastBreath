...
public class RescueEventHandler {
    ...
    public static void onRescueAttempt(PlayerInteractEvent.EntityInteract event) {
        ...
        if (!rescuer.isCrouching()) return;

        RescueManager.startRescue(rescuer, target);

        if (RescueManager.isBeingRescuedComplete(target)) {
        ...
        if (!player.level().isClientSide && player instanceof ServerPlayer serverPlayer) {
            ...
            if (RescueManager.isRescuing(player)) {
                ...
                if (RescueManager.shouldSyncProgress(player, progress)) {
                    PacketDistributor.sendToPlayer(Objects.requireNonNull(serverPlayer.server.getPlayerList().getPlayer(RescueManager.getTarget(serverPlayer.getUUID()))), new RescueProgressPayload(progress, RescueState.RESCUING));
                    PacketDistributor.sendToPlayer(serverPlayer, new RescueProgressPayload(progress, RescueState.RESCUING));
                }
                // 玩家取消救援（松开蹲键）
                if (!player.isCrouching()) {
                    PacketDistributor.sendToPlayer(Objects.requireNonNull(serverPlayer.server.getPlayerList().getPlayer(RescueManager.getTarget(serverPlayer.getUUID()))), new RescueProgressPayload(0.0f, RescueState.CANCELLED));
                ...
                if (RescueManager.shouldSyncProgress(player, progress)) {
                    PacketDistributor.sendToPlayer(Objects.requireNonNull(serverPlayer.server.getPlayerList().getPlayer(RescueManager.getRescuer(serverPlayer.getUUID()))), new RescueProgressPayload(progress, RescueState.RESCUING));
                    PacketDistributor.sendToPlayer(serverPlayer, new RescueProgressPayload(progress, RescueState.RESCUING));
                }
            } else if (RescueManager.isBeingRescuedComplete(player)) {
                PacketDistributor.sendToPlayer(Objects.requireNonNull(serverPlayer.server.getPlayerList().getPlayer(RescueManager.getRescuer(serverPlayer.getUUID()))), new RescueProgressPayload(1.0f, RescueState.COMPLETED));
                ...
                PacketDistributor.sendToPlayer(Objects.requireNonNull(serverPlayer.server.getPlayerList().getPlayer(RescueManager.getTarget(serverPlayer.getUUID()))), new RescueProgressPayload(1.0f, RescueState.COMPLETED));
                PacketDistributor.sendToPlayer(serverPlayer, new RescueProgressPayload(1.0f, RescueState.COMPLETED));
            }
        }
    }
